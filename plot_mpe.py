...
for alg in algos:
    filename = f"outputs_final/{dataset}/{alg}_*_28_%d_log.txt"
...
for alg in aggregate_results:
    for i, seed in enumerate(seed_list): 
        ...
        aggregate_results[alg][i] = savgol_filter(aggregate_results[alg][i][:, [0,11]], win_size, 1, axis=0)
...
ax.plot(mean_results["backpropODA"][:,0], mean_results["backpropODA"][:,-1], linewidth=l, color=tableau20blind[0], label="BODA")
ax.fill_between(mean_results["backpropODA"][:,0], mean_results["backpropODA"][:,-1] - var_results["backpropODA"][:,-1], mean_results["backpropODA"][:,-1] + var_results["backpropODA"][:,-1], color=tableau20blind[0], alpha = 0.3)

ax.plot(mean_results["domain_disc"][:,0], mean_results["domain_disc"][:,-1], linewidth=l, color=tableau20blind[1], label="Domain Disc")
ax.fill_between(mean_results["domain_disc"][:,0], mean_results["domain_disc"][:,-1] - var_results["domain_disc"][:,-1], mean_results["domain_disc"][:,-1] + var_results["domain_disc"][:,-1], color=tableau20blind[1], alpha = 0.3)

ax.plot(mean_results["kPU"][:, 0], mean_results["kPU"][:,-1], linewidth=l, color=tableau20blind[2], label="kPU")
ax.fill_between(mean_results["kPU"][:, 0], mean_results["kPU"][:,-1] - var_results["kPU"][:,-1], mean_results["kPU"][:,-1] + var_results["kPU"][:,-1], color=tableau20blind[2], alpha = 0.3)
...
plt.grid()
plt.savefig(f"plots/{dataset}_mpe.png" ,transparent=True,bbox_inches='tight')
plt.clf()